...
static void preencher_candidatos(ptrNo raiz, Point novo_ponto, Candidate *candidatos, int *quantidade)
{
    if (raiz == NULL) {
        return;
    }
...
{
    return (fabs(segmento.a.x - no->p.x) <= EPS && fabs(segmento.a.y - no->p.y) <= EPS) ||
           (fabs(segmento.b.x - no->p.x) <= EPS && fabs(segmento.b.y - no->p.y) <= EPS);
}

static bool validar_contra_segmentos(ptrNo raiz, ptrNo no_conexao, Segment novo_segmento, double distancia_minima)
{
    ...
    if (raiz->esq != NULL) {
        ...
        if (!segmento_incidente_ao_no(segmento_existente, no_conexao)) {
            ...
            if (distancia_segmentos(novo_segmento, segmento_existente) <= distancia_minima) {
                return false;
            }
        }

        if (!validar_contra_segmentos(raiz->esq, no_conexao, novo_segmento, distancia_minima)) {
            return false;
        }
    }
    ...
    if (raiz->dir != NULL) {
        ...
        if (!validar_contra_segmentos(raiz->dir, no_conexao, novo_segmento, distancia_minima)) {
            return false;
        }
    }
...
static bool conexao_valida(ptrNo raiz, ptrNo no_conexao, Point novo_ponto, double raio, double distancia_minima)
{
    ...
    return validar_contra_segmentos(raiz, no_conexao, novo_segmento, distancia_minima);
}
...
static bool construir_arvore(ptrNo raiz, int nterm, double raio, int *proximo_id, int *conexoes_rejeitadas)
{
    ...
    while (pontos_inseridos < nterm) {
        ...
        for (int tentativa = 0; tentativa < MAX_POINT_ATTEMPTS && !inseriu_ponto; tentativa++) {
            ...
            qsort(candidatos, (size_t)total_candidatos, sizeof(Candidate), comparar_candidatos);

            for (int i = 0; i < total_candidatos && !inseriu_ponto; i++) {
                ptrNo no_conexao = candidatos[i].no;

                if (conexao_valida(raiz, no_conexao, novo_ponto, raio, distancia_minima)) {
...
static bool ler_inteiro_positivo(const char *texto, int *valor)
{
    ...
    if (fim == texto || *fim != '\0' || convertido <= 0 || convertido > 1000000L) {
        return false;
    }
...
int main(int argc, char **argv)
{
    ...
    if (!ler_inteiro_positivo(argv[1], &nterm) || !ler_double_positivo(argv[2], &raio)) {
        imprimir_uso(argv[0]);
        return EXIT_FAILURE;
    }
